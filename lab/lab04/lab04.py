...
def distance(city1, city2):
    ...
    5.0
    """
    length = sqrt(abs((get_lat(city1) - get_lat(city2)))**2 + abs((get_lon(city1) - get_lon(city2))) **2 )
    return length
...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    Wolf = make_city('Wolf', lat, lon)
    distance1 = distance(Wolf,city1)
    distance2 = distance(Wolf,city2)
...
def put_piece(board, max_rows, column, player):
    ...
    """
    checkRow = max_rows - 1
    while checkRow >= 0 and get_piece(board,checkRow,column) != '-':
        checkRow -=1
    if checkRow >= 0:
        move = replace_elem(board[checkRow], column, player)
        new_board = replace_elem(board, checkRow, move)
        board = new_board
