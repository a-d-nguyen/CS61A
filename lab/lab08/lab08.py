...
def make_to_string(front, mid, back, empty_repr):
    ...
    def print(lnk):
        if lnk is Link.empty:
            ...
            return front + str(lnk.first) + mid + print(lnk.rest) + back
    return print
...
def add_trees(t1, t2):
    ...
    if not t2:
        return t1
    new_tree = t1.root + t2.root
    t1Branches, t2Branches = list(t1.branches), list(t2.branches)
    t1depth, t2depth = len(t1Branches), len(t2Branches)
    if t1depth < t2depth:
        t1Branches += [None for _ in range (t1depth, t2depth)]
    elif t1depth > t2depth:
        t2Branches += [None for _ in range (t2depth, t1depth)]
    return Tree(new_tree, [add_trees(branch1,branch2) for branch1, branch2 in zip(t1Branches, t2Branches)])
