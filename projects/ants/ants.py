...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        i = 0
        while location.entrance != None and location.entrance.name != 'Hive' and len(location.bees) == 0 and i < self.max_range:
            location = location.entrance
            i+=1
    ...
    def action(self, colony):
        ...
        if self.Imposter == True:
            ...
            while location:
                ...
                if location:
                    location_ant = location.ant
                    if location_ant:    
                        if isinstance(location_ant, BodyguardAnt):
                            if location_ant not in self.buff:
                                location_ant.damage *= 2
                                self.buff.append(location_ant)
                            location_ant = location_ant.ant
                        if location_ant and location_ant not in self.buff:
                            location_ant.damage *= 2
                            self.buff.append(location_ant)
