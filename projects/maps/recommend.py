...
def find_predictor(user, restaurants, feature_fn):
    ...
    b, a, r_squared =  S_xy/S_xx, mean(ys)-S_xy/S_xx*mean(xs), pow(S_xy, 2)/(S_xx*S_yy)  # REPLACE THIS LINE WITH YOUR SOLUTION
    # END Question 7
...
def rate_all(user, restaurants, feature_fns):
    ...
    for name in restaurants:
        RName = restaurant_name(name)
        if name in reviewed:
            rated[RName] = user_rating(user, RName)
        else:
            rated[RName] = predictor(name)
    return rated
    # END Question 9
