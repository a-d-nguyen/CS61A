...
def do_or_form(expressions, env):
    ...
    if scheme_truep(result) or expressions.second is nil:
        return result
    ...
    #     if scheme_truep(output) or expressions.second is nil:
    #         return output
    #     expressions = expressions.second
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if not scheme_symbolp(bindings.first.first):
            raise SchemeError
        new_frame.define(bindings.first.first, scheme_eval(bindings.first.second.first,env))
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra CreditW
            procedure = scheme_eval(first,env)
            expressions = rest.map(lambda operand: scheme_eval(operand,env))
            result = scheme_apply(procedure,expressions,env)
            # END Extra Credit
    return result
